...
async function proxy(request: Request, context: { params: Promise<{ path: string[] }> }) {
  ...
  const headers = new Headers(request.headers); headers.delete('host'); headers.set('X-AMP-Chat-Token', process.env.AMP_CHAT_TOKEN || '');
  ...
  const response = await fetch(target, init); const output = new Headers(response.headers); output.set('Cache-Control', 'no-cache'); output.set('X-Accel-Buffering', 'no');
